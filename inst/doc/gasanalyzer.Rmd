...
```
Small differences are found in variables as a result of the new $\alpha$ and in a value related to the energy balance. The latter is a result of a more precise conversion from Celcius to Kelvin units. Equations can be checked and modified before use, and custom equations can be added as `R` functions:
...
```{r eval=NOT_CRAN, fig.height=7, fig.width=7, purl=NOT_CRAN}
